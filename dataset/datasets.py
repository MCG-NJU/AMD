...
class SSRawFrameClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            return buffer, self.label_array[index], str(sample)
            ...
            buffer = self.data_transform(buffer)
            return buffer, self.test_label_array[index], str(sample), \
                   chunk_nb, split_nb
        else:
    ...
    def load_frame(self, sample, num_frames,sample_rate_scale=1):
        ...
        fname = os.path.join(self.data_root,str(sample))
        if self.mode == 'test':
            tick = num_frames / float(self.num_segment)
            temp_list = []
            temp_index_list = [[int(tick / y + tick * x) for x in range(self.num_segment)] for y in
                               range(2, self.test_num_segment + 1)]
            for sub_list in temp_index_list:
                temp_list += sub_list
            all_index = list(
                np.array([int(tick * x) for x in range(self.num_segment)] + temp_list
                         ))
...
class VideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_root, mode='train', clip_len=8,
        ...
        import pandas as pd
        cleaned = pd.read_csv(self.anno_path, header=None, sep=' ')
        self.dataset_samples = list(cleaned.values[:, 0])
        self.label_array = list(cleaned.values[:, -1])
...
class VideoMAE(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        conf_path = '~/petreloss_p.conf'
        self.video_loader = get_video_loader(conf_path=conf_path)
        
    def __getitem__(self, index):
        try:
            directory, _ = self.clips[index]
            video_name = os.path.join(self.root,directory)
...
class VideoMAE_SSV2(torch.utils.data.Dataset):
    def __init__(self,
                 root,
                 setting,
                 train=True,
        ...
        super(VideoMAE_SSV2, self).__init__()
        self.root = root
        self.setting = setting
        self.train = train
        self.test_mode = test_mode
        ...
        self.transform = transform
        self.lazy_init = lazy_init
        self.clip_len = new_length
        self.frame_sample_rate = new_step
        ...
        conf_path = '~/petreloss_p.conf'
        self.image_loader = get_image_loader(conf_path=conf_path)
    ...
    def load_frame(self, sample, num_frames, sample_rate_scale=1):
        # handle temporal segments
        fname = sample
        fname = os.path.join(self.root,str(sample))
        ...
        for idx in all_index:
            frame_fname = os.path.join(fname,
                                       self.name_pattern.format(idx + 1))
            img = self.image_loader(frame_fname)
            imgs.append(img)
        buffer = np.array(imgs)
        return buffer
    ...
    def __getitem__(self, index):
        try:
            scale_t = 1
            sample, total_frame, label = self.clips[index]
            buffer = self.load_frame(sample,
                                        total_frame,
                                        sample_rate_scale=scale_t)  # T H W C
            images = [transforms.ToPILImage()(frame) for frame in buffer]
            if self.num_sample > 1:
                process_data_list = []
                mask_list = []
                for _ in range(self.num_sample):
        ...
        with open(setting) as split_f:
            ...
            for line in data:
                ...
                if len(line_info) < 2:
                    raise (RuntimeError('Video input format is not correct, missing one or more element. %s' % line))
                clip_path = os.path.join(line_info[0])
                total_frames = int(line_info[1])
                target = int(line_info[-1])
                item = (clip_path, total_frames, target)
                clips.append(item)
        return clips
